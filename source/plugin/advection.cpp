...
//	std::cout << "In advectSemiLagrange , grid->is3D() = " << grid->is3D() <<" ; grid->is2D =" << grid->is2D() << " ; grid->getType() = " << grid->getType() << std::endl ; 
//        std::cout << "GridBase::TypeReal = " << GridBase::TypeReal << std::endl ;
//        std::cout << "GridBase::TypeMAC = " << GridBase::TypeMAC << std::endl ;
//        std::cout << "GridBase::TypeLevelset = " << GridBase::TypeLevelset << std::endl ;
...
//! Perform semi-lagrangian advection of target Real- or Vec3 grid
PYTHON void advectSemiLagrange_time_fraction (FlagGrid* flags, MACGrid* vel, GridBase* grid, 
						   int order = 1, Real strength = 1.0, Real time_fraction = 1.0)
{    
	assertMsg(order==1 || order==2, "AdvectSemiLagrange: Only order 1 (regular SL) and 2 (MacCormack) supported");
	
        // determine type of grid    
	if (grid->getType() & GridBase::TypeReal) {
		fnAdvectSemiLagrange_time_fraction< Grid<Real> >(flags->getParent(), *flags, *vel, *((Grid<Real>*) grid), order, strength, time_fraction);
	}
	else if (grid->getType() & GridBase::TypeMAC) {    
		fnAdvectSemiLagrange_time_fraction< MACGrid >(flags->getParent(), *flags, *vel, *((MACGrid*) grid), order, strength, time_fraction);
	}
	else if (grid->getType() & GridBase::TypeVec3) {    
		fnAdvectSemiLagrange_time_fraction< Grid<Vec3> >(flags->getParent(), *flags, *vel, *((Grid<Vec3>*) grid), order, strength, time_fraction);
	}
	else
		errMsg("AdvectSemiLagrange: Grid Type is not supported (only Real, Vec3, MAC, Levelset)");    
}
