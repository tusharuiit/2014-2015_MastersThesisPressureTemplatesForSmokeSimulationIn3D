...
//! of input grids.
KERNEL 
void knApplyNoiseVec_custom_velocity_template (FlagGrid& flags, Grid<Vec3>& target, WaveletNoiseField& noise, 
					  Real scale, Real scaleSpatial, Grid<Real>* weight, Grid<Vec3>* uv, bool uvInterpol, const Vec3& sourceFactor, Shape* shaper = NULL ) 
{
	if ( !flags.isFluid(i,j,k) ) return;
	...
	Vec3 noiseVec3 = noise.evaluateCurl( pos ) * scale * w; 
	//noiseVec3=pos; // debug , show interpolated positions
	if (!(shaper->isInsideGrid(i,j,k))) {target(i,j,k) += noiseVec3;}
} 

PYTHON void applyNoiseVec3_custom_velocity_template ( FlagGrid& flags, Grid<Vec3>& target, WaveletNoiseField& noise, 
							Real scale=1.0 , Real scaleSpatial=1.0 , Grid<Real>* weight=NULL , Grid<Vec3>* uv=NULL , Shape* shaper = NULL )
{
	// check whether the uv grid has a different resolution
	bool uvInterpol = false; 
	// and pre-compute conversion (only used if uvInterpol==true)
	...
	// note - passing a MAC grid here is slightly inaccurate, we should evaluate each component separately
	knApplyNoiseVec_custom_velocity_template(flags, target, noise, scale, scaleSpatial, weight , uv,uvInterpol,sourceFactor, shaper );
}
