...
//	inline T get(const Vec3i& pos) const           { return mData[index(pos)]; }
	// Tushar
	PYTHON inline T get(const Vec3i& pos) const           { return mData[index(pos)]; }	
	//! access data
	inline T& operator()(int i, int j, int k)      { return mData[index(i, j, k)]; }
	PYTHON void assign_value(int idx_x,int idx_y,int idx_z,Real value)
	{
		(*this)(idx_x,idx_y,idx_z) = value;
	}
	//! access data
