...
Real fmin( Real a, Real b)
{
    if( a < b ) return a;
    return b;
}

Real fmax( Real a, Real b)
{
    if( a > b ) return a;
    return b;
}
...
void read_Real( const char* szFileName, const char* szVarName, Real* pVariable)
{
...
void write_matrix( const char* szFileName,       /* filename */
		   ...
		   int ncl,		       /* first row */
		   int nch,		       /* last row */
		 Real xlength,	       /* size of the geometry in */
                                               /* x-direction */
		 Real ylength,	       /* size of the geometry in */
                                               /* y-direction  */
...
Real **matrix( int nrl, int nrh, int ncl, int nch )
{
   ...
   Real **pArray  = (Real **) malloc((size_t)( nrow * sizeof(Real*)) );
   Real  *pMatrix = (Real *)  malloc((size_t)( nrow * ncol * sizeof( Real )));
...
{
   Real **pArray  = m + nrl;
   Real  *pMatrix = m[nrl]+ncl;

   free( pMatrix );
   free( pArray );
}

void init_matrix( Real **m, int nrl, int nrh, int ncl, int nch, Real a)
{
   int i,j;
   for( i = nrl; i <= nrh; i++)
       for( j = ncl; j <= nch; j++)
