...
//// Temporary test by Tushar
//// This will set the values of residual at the target points to be zero
////KERNEL(bnd=1)
//void modify_residual_to_target(Grid<Real>& residual , Vec3 splat_center = Vec3(31,31,0.5) , int splat_radius = 3)
//{
...
//     idx = residual.index(idx_x , idx_y , idx_z);
//    
//     int temp_idx_x , temp_idx_y ;

////   force the residual to zero at those target points of the splat
//     for (int x = -(splat_radius); x <= (splat_radius); x++)
//     {
//         for (int y = -(splat_radius); y <= (splat_radius); y++)
//         {
//             temp_idx_x = idx_x + x ;
//             temp_idx_y = idx_y + y ;
//             idx = residual.index(temp_idx_x , temp_idx_y , idx_z);
//             residual[idx] = 0.0;
//         }
//     }
// }
...
//// Temporary test by Tushar
//// This will set the values of pressure at the target points to be target values
////KERNEL(bnd=1)
//void modify_pressure_to_target(Grid<Real>& pressure , Vec3 splat_center = Vec3(31,31,0.5) , int radius = 3)
//{
//     int idx , idx_x , idx_y , idx_z;
//     idx_x = (toVec3i(splat_center)).x;
//     idx_y = (toVec3i(splat_center)).y;
...
//     double sum = 0.0;
//     idx = pressure.index(idx_x , idx_y , idx_z);
...
//             temp_idx_y = idx_y + y ;
//             r = sqrt(x*x + y*y);
//             idx = pressure.index(temp_idx_x , temp_idx_y , idx_z);
//             pressure[idx] = (exp(-(r*r)/s))/(M_PI * s);
//             sum += pressure[idx];
//         }
//     }
// 
...
//     idx = pressure.index(idx_x , idx_y , idx_z);
//     Real central_value = pressure[idx] ;
...
//              temp_idx_y = idx_y + j ;
//              idx = pressure.index(temp_idx_x , temp_idx_y , idx_z);
//              pressure[idx] /= central_value;
//         }
//     }

////   At this point , the pressure contains a gaussian kernel at the target points
//     for(int i = -(radius); i <= (radius); ++i)
//     {
//         for(int j = -(radius); j <= (radius); ++j)
//         {        
...
//	modify_pressure_to_target(mDst , splat_center , splat_radius);
//	modify_residual_to_target(mResidual , splat_center , splat_radius);
...
//		Print statements by Tushar
//		std::cout << "mResNorm<mAccuracy" << std::endl ;		
//		std::cout << "mResNorm = " << mResNorm << std::endl ;
//		std::cout << "mAccuracy = " << mAccuracy << std::endl ;
//		std::cout << "vector epsilon = " << VECTOR_EPSILON <<std::endl ;
