 ...
 * READ_STRING( szFile, szProblem );
 */
#define READ_Real( szFileName, VarName) read_Real( szFileName, #VarName, &(VarName) )
...
void read_Real( const char* szFilename, const char* szName, Real*  Value);
 ...
 *                    RESET_TIMER()
 */
#define DUMP_Real(d)  fprintf( DUMPOUT, "%s:%d %s = %f\n", __FILE__, __LINE__, #d, d )
