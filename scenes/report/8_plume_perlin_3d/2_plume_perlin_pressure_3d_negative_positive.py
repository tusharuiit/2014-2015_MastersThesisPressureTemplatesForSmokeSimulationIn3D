...
#source = s.create(Cylinder, center=gs*vec3(0.5,0.15,0.5), radius=res*0.20, z=gs*vec3(0, 0.025, 0))

#simulation_directory_name = "/media/tushar/B19D-ABF3/latex1/6_3D_april_11/11_pressure_template_rotation/" + folder_simulation_number
#vol_directory_name = "/media/tushar/B19D-ABF3/latex1/6_3D_april_11/11_pressure_template_rotation/" + folder_simulation_number

simulation_directory_name = "/media/tushar/TOSHIBA EXT/5_pressure_positive_negative_6/screenshot"
vol_directory_name = "/media/tushar/TOSHIBA EXT/5_pressure_positive_negative_6/vol"
...
#		densityInflow(flags=flags, density=density, noise=noise, shape=source, scale=1, sigma=0.5)
	...
	if ( (t>=t1) and (t<=t2) and ((t % every_time_step == 0)) ):
		...
		if((t % (alternator) == 0)):
			...
			sign_changer = 1
		create_perlin_template_3D_kernel ( pressure_template_perlin_x, multiplier=(multiplier*sign_changer), res_x=gs_x, res_y=gs_y, res_z=gs_z, levels=6, time=t, frequency_x=x_frequency_x, frequency_y=x_frequency_y, frequency_z=10, translation_x=-0.05, translation_y=-0.05, translation_z=-0.05 )
...
##		Apply the template
		apply_template_density_localized_shape ( flags=flags, vel=vel, pressure_template = pressure_template_perlin_x, density=density, shape = source, density_threshold=-0.00001, density_based_gradient_scaling=1 )
