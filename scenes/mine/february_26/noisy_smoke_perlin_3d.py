...
#	advectSemiLagrange(flags=flags, vel=vel, grid=vel    , order=2, strength=1.0)
	...
	t2 = 301
	if (t>=t1)and(t<t2):
		create_perlin_template_3_3D_kernel_linear(pressure_template_perlin,multiplier=6.0,res_x=gs_x,res_y=gs_y,res_z=gs_z,levels=6,time=t,start_x=15,start_y=5,start_z=10,time_steps=60)
...
#	solvePressure(flags=flags, vel=vel, pressure=pressure, useResNorm=True)
	...
	density.save('/media/tushar/TOSHIBA EXT/feb_26/1/den%04d.vol' % t)
