...
create_pressure_modifier_elliptical_gaussian_with_multiplier(pressure_modification_elliptical_gaussian , splat_radius_x , splat_radius_y , splat_center_ellipse, splat_sigma_x, splat_sigma_y , multiplier = 0.05, lower_factor = 1.0, higher_factor = 1.0)
...
##	In this we are advecting density first, and then velocity
...
#	In this we are advecting velocity first, and then density
	advectSemiLagrange_time_fraction(flags=flags, vel=vel, grid=density, order=2, time_fraction = 1.0)
	advectSemiLagrange_time_fraction(flags=flags, vel=vel, grid=vel, order=2, time_fraction = 1.0)
	setWallBcs(flags=flags, vel=vel)    
...
#	setWallBcs(flags=flags, vel=vel)
	...
	timings.display()
	s.step()
#	gui.screenshot( '/home/tushar/manta_mikey_1/manta/build/pictures/december_11_12/center_constant/0.25/%03d_dens.png' % t )
