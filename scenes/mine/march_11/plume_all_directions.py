...
#curl_strength_modifier=float((sys.argv[11]))
...
for t in range(gs_z):
	...
	if ((t>=t1)and(t<t2)):
		...
		create_perlin_template_26(pressure_template_perlin_x,multiplier=multiplier*0.75,res_x=gs_x,res_y=gs_y,res_z=gs_z,frequency_x=x_frequency_x,frequency_y=x_frequency_y,frequency_z=frequency_z,z=t,folder_templates_number=folder_templates_number)
		create_perlin_template_26(pressure_template_perlin_y,multiplier=multiplier*3.0,res_x=gs_x,res_y=gs_y,res_z=gs_z,frequency_x=y_frequency_x,frequency_y=y_frequency_y,frequency_z=frequency_z,z=t,folder_templates_number=folder_templates_number)

#	Interpolate
#    We move from x:y = start_x:start_y to x:y start_y:start_y

		time_depender = ((t - 1) % time_steps)
		time_depender_2 = ((t - 1) % time_steps_2)
		if(time_depender_2 < time_steps):
			sign_factor = -1.0
		else:
			sign_factor = 1.0
		if(time_depender < (time_steps/2)):
			linear_interpolate = (start_x + ((start_y - start_x)/(time_steps/2.0)) * time_depender)*sign_factor
			...
			linear_interpolate = (start_y + ((start_x - start_y)/(time_steps/2.0)) * time_depender)*sign_factor
			value_to_write = str(t) + " linear_interpolate = " + str(linear_interpolate) + "\n"
			file_pointer.write(value_to_write)

		pressure_template_perlin_x.multConst(linear_interpolate)
		pressure_template_perlin_y.multConst((1.0*sign_factor - linear_interpolate))
		...
		pressure_template_perlin_x_y.multConst(1.0)
