...
#	If you want to linearly increase the value from start_value to end_value
#	m = ((float)(end_value - start_value))/(t2 - t1)
#	c = (float(t2*start_value - t1*end_value))/(t2 - t1)
#	Linear from -1 to 6
	start_value = (0.0)
	end_value = 6.0
...
#	time range of application of pressure modification
	...
	if (t>t1)and(t<t2):
		...
		fraction = math.pow(fraction,3.0)
		print " fraction is "
		print fraction
		modify_pressure_constant(pressure,pressure_modification_elliptical_gaussian,multiplier=(end_value * fraction))
	...
	s.step()
	gui.screenshot( './pictures/november_3/4.2/%03d_density.png' % t )
